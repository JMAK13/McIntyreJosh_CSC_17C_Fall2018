...
Card::Card(int v) : AbstractCard(){
    ...
    try{
        if(serial>=1&&serial<=13) data.second="Spades";
        else if(serial>=14&&serial<=26) data.second="Clubs";
        else if(serial>=27&&serial<=39) data.second="Hearts";
        else if(serial>=40&&serial<=52) data.second="Diamonds";
    }
